...
/** 产业链节点关系（节点间的流向关系） */
export interface ChainNodeRelation {
  id: string;
  /** 源节点ID（上游） */
  sourceNodeId: string;
  /** 目标节点ID（下游） */
  ...
  /** 所属产业链ID */
  chainId: string;
  /** 关系类型：flow=物流/产品流, info=信息流, fund=资金流 */
  relationType: 'flow' | 'info' | 'fund';
  /** 关系描述，如"正极材料供应"、"信息共享" */
  description?: string;
  /** 关系强度 1-100 */
  strength?: number;
...
export interface ChainStructure {
  ...
  tree: ChainTreeNode;
  /** 所有节点的位置映射（用于图谱模式） */
  nodePositions?: Record<string, { x: number; y: number; size: number; category: string }>;
  /** 所有连线关系 */
  relations?: ChainNodeRelation[];
  /** 链上企业汇总信息 */
  chainFirms?: Array<{
    nodeId: string;
    firmId: string;
